...
class tool_coursearchiver_processor {
    ...
    public function __construct(array $options) {
        ...
        $this->reset();
        $this->runid = time(); // Unique per archive execution 
    }
  ...
  protected function archivecourse($obj, $delete = true) {
    ...
    try {
        ...
        $archivepath = trim(
            str_replace(str_split(':*?"<>|'), '', get_config('tool_coursearchiver', 'coursearchiverpath')),
            "/\\"
        );
        ...
        if (!empty($course->idnumber)) {
            $suffix .= '-IDNUM-' . $course->idnumber;
        }

        $dirtyname = date("Y-m-d") . $suffix . "-" . $course->shortname . ".mbz";
        $archivefile = preg_replace(
            ['/\s/', '/\//', '/\;/', '/\:/', '/\?/', '/\%/', '/\*/', '/\|/', '/\</', '/\>/'],
            '-',
            $dirtyname
        );

        $folder = $this->get_course_term_folder($course);
        $path = $rootpath . '/' . $archivepath . '/' . $folder;

        if (!is_dir($path) && !mkdir($path, $CFG->directorypermissions, true)) {
            throw new Exception('Failed to create archive directory');
        }
        ...
        if (empty($results['backup_destination'])) {
            throw new Exception('Backup destination missing');
        }

        $fullfile = $path . '/' . $archivefile;
        $results['backup_destination']->copy_content_to($fullfile);
        ...
        if (!$DB->get_record('tool_coursearchiver_archived', ['filename' => $folder . '/' . $archivefile])) {
            $record = new stdClass();
            $record->filename = $folder . '/' . $archivefile;
            $record->owners = '';
            $record->timetodelete = 0;
            $DB->insert_record('tool_coursearchiver_archived', $record);
        }

        // Log SUCCESS.
        $this->log_archive_result(
            $course->id,
            $course->fullname,
            'success',
            $folder . '/' . $archivefile,
        ...
        if ($delete) {
            ...
            $task->set_custom_data(['course' => $course]);
    ...
    /**
 * Log archive result per course.
 *
 * @param int $courseid
 * @param string $coursename
 * @param string $status success|failed
 * @param string|null $backupfile
 * @param string|null $errormessage
 */
protected function log_archive_result(
    int $courseid,
    string $coursename,
    string $status,
    ?string $backupfile,
    ...
    $record->backupfile   = $backupfile;
    $record->errormessage = $errormessage;
    $record->timecreated  = time();

    $DB->insert_record('tool_coursearchiver_log', $record);
}
    ...
    protected function find_latest_file($files) {
     ...
     /**
 * Get archive folder name based on course custom field "term".
 *
 * @param stdClass $course
 * @return string
 */
protected function get_course_term_folder(stdClass $course): string {
    global $DB;
    ...
    $sql = "
        SELECT COALESCE(d.value, d.charvalue, d.shortcharvalue) AS term
    ...
    if ($record = $DB->get_record_sql($sql, $params)) {
        if (!empty($record->term)) {
            return preg_replace('/[^a-zA-Z0-9\-_]/', '-', $record->term);
        }
    }
...
protected function get_archive_folder() {
    // If folder explicitly set (manual override), respect it.
    if (!empty($this->folder)) {
        return str_replace(str_split('\\/:*?"<>|'), '', $this->folder);
    }

    // Folder is determined per course during archive.
    // Default fallback.
    return date('Y');
}
...
public function get_courselist() {
    ...
    foreach ($this->data as $key => $value) {
        if (!empty($value)) {
            ...
            // ✅ Handle custom field 'term' (added filter searches[term]).
            if ($key === 'term' || $key === 'searches_term') {
                // Accept comma-separated values like "2025-Fall,2025-Spring".
                $termlist = array_filter(array_map('trim',
                    preg_split('/\s*,\s*/', (string)$value)
                ));

                if (!empty($termlist)) {
                    // Bind 'term' shortname and IN() list.
                    $params['termshort'] = 'term';
                    list($insql, $inparams) = $DB->get_in_or_equal($termlist, SQL_PARAMS_NAMED, 'termv');
                    $params = array_merge($params, $inparams);

                    // ⚠️ Note: component/area are on customfield_category (alias cc), not on field.
                    // Value may be stored in value/charvalue/shortcharvalue depending on Moodle version/type.
                    $searchsql .= "
                        AND c.id IN (
                            SELECT d.instanceid
                              FROM {customfield_field} f
                             ...
                             WHERE cc.component = 'core_course'
                               AND cc.area      = 'course'
                               AND f.shortname  = :termshort
                               AND COALESCE(d.value, d.charvalue, d.shortcharvalue) $insql
                        )
                    ";
                }

                // Skip normal handling for this key.
                continue;
            }

            // 🔽 Existing logic for all other built-in filters (unchanged).
...
/**
 * Schedule a deletion (soft delete).
 *
 * Marks archive files for deletion after a delay.
 * Does NOT delete files immediately.
 *
...
public static function delete_archives($selected) {
    ...
    // 🔒 Defensive default: 7 days if not configured
    $delaydelete = isset($config->delaydeletesetting)
        ? (int)$config->delaydeletesetting
        : 7;

    foreach ($selected as $course) {
        $fullpath = $rootpath . '/' . $archivepath . '/' . $course;

        if (!file_exists($fullpath)) {
            continue;
        }

        // Calculate scheduled deletion time safely
        $time = new DateTime('now', core_date::get_user_timezone_object());
        $time->modify('+' . $delaydelete . ' days');
 ...
 /**
 * Cancel scheduled deletion of archived files.
 *
 * @param array $selected collection of filenames selected.
 * @return void
 */
